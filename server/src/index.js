...
import { getActiveSessionsForUser, createFamily, createSession, findSessionByRefreshLookup, getEvents, insertEvent, markFamilyCompromised, revokeFamily, updateSessionRefresh, revokeSession, seedUser, hashUserAgentIP, db, nowISO } from './db';
...
app.post('/login', async (c) => {
    try {
        ...
        const { token: accessToken, jti } = await signAccessToken({ sub: String(user.id), username: user.username }, accessTTL);
        ...
        insertEvent('LOGIN_SUCCESS', user.id, sessionId, `Session created for ${user.username}`, ip, geoData.country_code, geoData.country, geoData.city, geoData.isp, // Use geoData.isp instead of geoData.org
        geoData.latitude === null ? undefined : geoData.latitude, geoData.longitude === null ? undefined : geoData.longitude);
        return c.json({
            accessToken,
...
app.post('/signup', async (c) => {
    try {
        ...
        const userId = db.query('SELECT last_insert_rowid() as id').get();
